(ns mitid_denmark.main-test
  "Contract + behavioral test for the mitid_denmark-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "mitid_denmark-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'mitid_denmark.main-test)]
