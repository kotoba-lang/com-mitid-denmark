(ns mitid_denmark.main
  "Kotodama WASM entrypoint for the Mitid_denmark clean-room actor (L4) — Clojure port.
  ...
  facts (`mitid_denmark.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mitid_denmark")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mitid_denmark.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mitid_denmark-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mitid_denmark-compat/ready)
